import { getSequence, queueJob } from '../../shared/src/index';
import { OptionsComponent, ComponentsComponent } from './type';
import { effect, reactive } from '../../reactivity/src/index';
import { ObjectOfStringKey } from 'packages/shared/src/types';

interface Props extends Partial<Omit<HTMLElement, 'className' | 'style'>> { 
...
type componentInstance = {
  ...
  state: ObjectOfStringKey,
  subTree: VNODE | null,
};

type ComponentNode = {
  el: EL,
  type: OptionsComponent/*  | ComponentsComponent */,
  key?: string,
  props?: Props,
...
export type VNODE = ComponentNode | HTMLNode;
...
export function createRenderer(options?: Options) { 
  ...
  function resolveProps(options: ObjectOfStringKey = {}, propsData: ObjectOfStringKey = {}): {props: ObjectOfStringKey, attrs: ObjectOfStringKey} { 
    const props = {};
    const attrs = {};
  ...
  function getRenderContext(instance: componentInstance) { 
    return new Proxy(instance, {
      ...
      set(target, key, value) { 
        ...
        if (Reflect.has(props, key)) {
          console.warn(`直接设置props可能导致组件更新失败: ${key}`);
  ...
  function mountComponent(node: ComponentNode, container: Container, anchor?: HTMLElement | ChildNode | null) {
    ...
    const { props } = resolveProps(propsOptions, node.props);

    beforeCreate && beforeCreate();
    let state: object;
    if ('data' in component) { 
      state = reactive(component.data());
    } else { 
      state = {};
    }

    // 添加组件示例，用来维护组件状态，并将props 及 data 转换为响应式对象，这样即使直接修改实例也可以完成组件更新
    node.component = {
    ...
    const renderContext = getRenderContext(node.component);
    created && created(renderContext);
    
    effect(() => { 

      const subTree = component.render?.call(renderContext, renderContext);
      if (!subTree) throw Error('');
  ...
  function patchComponent(oldVNode: ComponentNode, newVNode: ComponentNode) {
    const instance = (newVNode.component = oldVNode.component);

    if (!hasPropsChanged(oldVNode.props, newVNode.props)) return;

    const props = instance.props;
    const { props: nextProps } = resolveProps(newVNode.type.props, newVNode.props);

    for (const key in nextProps) {
      Reflect.set(props, key, nextProps[key]);
    }
  ...
  function hasPropsChanged(oldProps: OptionsComponent['props'], newProps: OptionsComponent['props']): boolean { 
  ...
  function patch(oldVNode: VNODE | null, newVNode: VNODE, container: Container, anchor?: HTMLElement | ChildNode | null) { 
    ...
    if (typeof newVNode.type === 'string') {
      if (!oldVNode) {
        ...
        patchElement(newVNode, oldVNode, container, anchor);
      }
    }
    
    if (isComponentNode(newVNode)) { 
      if (!oldVNode) {
        mountComponent(newVNode, container, anchor);
      } else { 
        patchComponent(oldVNode as ComponentNode, newVNode);
      }
    }
